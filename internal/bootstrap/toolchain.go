...
func DefaultTools() map[string]string {
	return map[string]string{
		// The environment layer, so the box feels like the machine the operator
		// already works on: a prompt, the dotfiles that build it, the search tools,
		// shell history, the agent-pane server, and the secrets CLI.
